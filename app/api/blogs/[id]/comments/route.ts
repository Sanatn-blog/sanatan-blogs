...
async function createCommentHandler(
  ...
  try {
    ...
    if (parentCommentId) {
      ...
      const { createReplyNotification } = await import("@/lib/notifications");
      ...
      // Create notification for blog author (top-level comment)
      const { createCommentNotification } = await import("@/lib/notifications");
