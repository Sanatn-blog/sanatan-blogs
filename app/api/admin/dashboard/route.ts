...
// Force dynamic rendering and disable caching for fresh dashboard data
...
async function getDashboardDataHandler() {
  try {
    ...
    const blogGrowthPercent =
      newBlogsYesterday > 0
        ? Math.round(
            ((newBlogsToday - newBlogsYesterday) / newBlogsYesterday) * 100,
          )
        : newBlogsToday > 0
          ? 100
          : 0;
    ...
    const commentGrowthPercent =
      newCommentsYesterday > 0
        ? Math.round(
            ((newCommentsToday - newCommentsYesterday) / newCommentsYesterday) *
              100,
          )
        : newCommentsToday > 0
          ? 100
    ...
    return NextResponse.json({
      ...
      summary: {
        ...
        engagementRate:
          totalUsers > 0 ? Math.round((totalComments / totalUsers) * 100) : 0,
        averageViewsPerBlog:
          totalBlogs > 0 ? Math.round(totalViewsCount / totalBlogs) : 0,
