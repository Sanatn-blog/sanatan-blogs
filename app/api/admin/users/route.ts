...
// Force dynamic rendering and disable caching for fresh user list
...
async function updateUserStatusHandler(request: AuthenticatedRequest) {
  try {
    ...
    console.log(
      `Admin ${request.user?.email} changed user ${user.email} status to ${status}`,
    );
...
async function deleteUserHandler(request: AuthenticatedRequest) {
  try {
    ...
    console.log(
      `Super admin ${request.user?.email} deleted user ${user.email}`,
    );
...
async function createUserHandler(request: AuthenticatedRequest) {
  try {
    ...
    console.log(
      `Admin ${request.user?.email} created user ${newUser.email} with role ${newUser.role}`,
    );
