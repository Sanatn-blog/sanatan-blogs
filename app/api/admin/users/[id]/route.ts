...
// Force dynamic rendering and disable caching for fresh user details
...
async function getUserHandler(
  ...
  try {
    ...
    const user = await User.findById(userId).select(
      "-password -emailVerificationToken -resetPasswordToken -resetPasswordExpires -otp -otpExpiry",
    );
  ...
  try {
    ...
    if (userBlogs.length > 0) {
      return NextResponse.json(
        {
          error:
            "Cannot delete user with existing blogs. Please delete or transfer their blogs first.",
        },
        { status: 400 },
      );
    }
...
async function patchUserHandler(
  ...
  try {
    ...
    console.log(
      `User updated by ${request.user?.role}:`,
      updatedUser?.email,
      "Role:",
      updatedUser?.role,
      "Status:",
